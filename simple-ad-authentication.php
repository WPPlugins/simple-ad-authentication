<?php
/*
Plugin Name: Simple AD Authentication
Version: 0.8
Plugin URI: http://simple-ad-authn.sourceforge.net
Description: Allows WordPress to authenticate users through Active Directory
Author: Drahkar
Author URI: http://drahkar.users.sourceforge.net/
*/

/*      Copyright 2009 James Boylan
...
if ( !class_exists('adAuthenticationPlugin') ) {

	class adAuthenticationPlugin {
		var $authenticated = false;
		var $ldap_auth_domain = 'ldap-auth';
		
		function adAuthenticationPlugin() {
		...
		function initialize_options() {
			if ( current_user_can('manage_options') ) {
				add_option('LDAP_authentication_auto_create_user', true, 'Should a new user be created automatically if not already in the WordPress database?');
				...
				add_option('LDAP_authentication_uid_filter', '(samaccountname=%user_id%)', 'AD User search filter');
				add_option('LDAP_authentication_group_filter', '(cn=%group%)', 'LDAP group search filter');
				add_option('LDAP_authentication_group_attribute', 'memberOf', 'LDAP group attribute');
			}
		}
		...
		function add_options_page() {
			if ( function_exists('add_options_page') ) {
				$page = add_options_page(__('Active Directory Authentication', $this->ldap_auth_domain), __('Active Directory Authentication', $this->ldap_auth_domain), 9, $this->ldap_auth_domain, array(&$this, '_display_options_page'));
		...
		function authenticate( $username, $password ) {
			...
			if ( $bind_dn ) {
				if ( !defined('WP_DEBUG') || ( defined('WP_DEBUG') && false === WP_DEBUG ) ) {
					...
					trigger_error('Active Directory bind as \'' . $bind_dn . '\'.');
					$ldap_bind = ldap_bind($ldap, $bind_dn, $bind_password);
				}
				if ( !$ldap_bind ) {
					if ( $use_ssl )
						return new WP_Error('cannot_bind', __('<strong>ERROR</strong>: Active Directory bind failed. Either the LDAPS connection failed or the login credentials are incorrect.', $this->ldap_auth_domain));
					else
						return new WP_Error('cannot_bind', __('<strong>ERROR</strong>: Active Directory bind failed. Check the login credentials.', $this->ldap_auth_domain));
				}
			} else {
				if ( !defined('WP_DEBUG') || ( defined('WP_DEBUG') && false === WP_DEBUG ) ) {
					$ldap_bind = @ldap_bind($ldap);
				} else {
					trigger_error('Active Directory bind as anonymous.');
					$ldap_bind = ldap_bind($ldap, $bind_dn, $bind_password);
				}
				if ( !$ldap_bind ) {
					if ( $use_ssl )
						return new WP_Error('cannot_bind', __('<strong>ERROR</strong>: Anonymous Active Directory bind failed. Either the Active Directory SSL connection failed or the login credentials are incorrect.', $this->ldap_auth_domain));
					else
						return new WP_Error('cannot_bind', __('<strong>ERROR</strong>: Anonymous Active Directory bind failed. Check the login credentials.', $this->ldap_auth_domain));
				}
			}
			if ( !defined('WP_DEBUG') || ( defined('WP_DEBUG') && false === WP_DEBUG ) ) {
			...
			if ( !defined('WP_DEBUG') || ( defined('WP_DEBUG') && false === WP_DEBUG ) ) {
				...
				trigger_error('Active Directory re-bind as \'' . $ldap_user . '\'.');
				$ldap_bind = ldap_bind($ldap, $ldap_user, $password);
			}
			if ( $ldap_bind ) {
			...
			if ( !$user or $user->user_login != $username ) {
				...
				if ( (bool) get_option('LDAP_authentication_auto_create_user' )
					...
					@ldap_unbind($ldap);
					return new WP_Error('invalid_username', __('<strong>ERROR</strong>: This user exists in Active Directory, but has not been granted access to this installation of WordPress.', $this->ldap_auth_domain));
				}
			}
		...
		function _user_in_group( $ldap, $username, $group ) {
			...
			if ( $replace_count == 0 ) {
				if ( defined('WP_DEBUG') && ( true === WP_DEBUG ) )
					trigger_error('Active Directory group search filter is mistaked.');
				return false;
			}
			if ( !defined('WP_DEBUG') || ( defined('WP_DEBUG') && false === WP_DEBUG ) ) {
				$result = @ldap_search($ldap, $base_dn, $group_filter, array($group_attr));
			} else {
				trigger_error('Searching group in Active Directory with \'' . $group_filter . '\' filter.');
				$result = ldap_search($ldap, $base_dn, $group_filter, array($group_attr));
			}
...
<div class="wrap">
	<h2><?php _e('Active Directory Authentication Options', $this->ldap_auth_domain); ?></h2>
	<form action="options.php" method="post">
		...
		<input type="hidden" name="LDAP_authentication_use_ssl" id="LDAP_authentication_use_ssl" value="0" />
		<?php if (function_exists('wp_nonce_field')): wp_nonce_field('update-options'); endif; ?>

		<table class="form-table">
			<tr valign="top">
				<th scope="row"><label for="LDAP_authentication_server"><?php _e('Active Directory Server', $this->ldap_auth_domain); ?></label></th>
				<td>
					<input type="text" name="LDAP_authentication_server" id="LDAP_authentication_server" value="<?php echo $ldap_server; ?>" /><br />
					<?php _e('Active Directory Server (e.g. <code>ad.example.net</code>)', $this->ldap_auth_domain); ?>
				</td>
			</tr>
			<tr valign="top">
				...
				<td>
					...
					<?php _e('Base DN (e.g., <code>ou=users,dc=example,dc=net</code>)<br />This should be the branch where you store users for Active Directory<br />DO NOT use the root DN. It will error out on the ldap search.', $this->ldap_auth_domain); ?><br />
				</td>
			</tr>
			<tr valign="top">
				...
				<td>
					...
					<?php _e('Bind DN (e.g., <code>cn=proxyuser,dc=example,dc=net</code> or<br />e.g., <code>proxyuser@example.net</code>)', $this->ldap_auth_domain); ?><br />
				</td>
			</tr>
			<tr valign="top">
			...
			<tr valign="top">
				...
				<td>
					...
					<?php _e('Filter for searching user ID (e.g., <code>(samaccountname=%user_id%)</code>)<br />
This setting must contain <code>%user_id%</code> string.', $this->ldap_auth_domain); ?>
			...
			<tr valign="top">
				...
				<td>
					...
					<?php _e('Active Directory filter for searching group (e.g., <code>(cn=%group%)</code>)<br />
This setting must contain <code>%group%</code> string.', $this->ldap_auth_domain); ?>
			...
			<tr valign="top">
				...
				<td>
					...
					<?php _e('Active Directory attribute for group member (e.g., <code>memberOf</code>)', $this->ldap_auth_domain); ?>
				</td>
			</tr>
			<tr valign="top">
				<th scope="row"><label for="LDAP_authentication_auto_create_user"><?php _e('Automatically create accounts for any and all users can authenticate to the Active Directory?', $this->ldap_auth_domain); ?></label></th>
				<td>
...
<strong>Users with role equivalent groups will be created even if this setting is turned off</strong> (because if you didn\'t want this to happen, you would leave that option blank.)'); ?>
			...
			<tr valign="top">
				...
				<td>
					...
					<?php _e('If the Active Directory attribute \'mail\' is blank, a user\'s email will be set to username@whatever-this-says', $this->ldap_auth_domain); ?>
				</td>
			</tr>
			<tr valign="top">
...
Format: <code>AD-Group=WordPress-Role;AD-Group=WordPress-Role;...</code><br />
...
ALSO NOTE: AD groups are case-sensitive', $this->ldap_auth_domain); ?>
				</td>
			</tr>
		</table>
	...
	<table class="equivalent-table form-table">
		<thead>
			<tr align="center" class="ui-widget-header">
				<th scope="col"><?php _e('AD Group', $this->ldap_auth_domain); ?></th>
...
$Ldap_authentication_plugin = new adAuthenticationPlugin();
?>
